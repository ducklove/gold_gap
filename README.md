...
선택 기간의 국내 가격 변화를 로그수익률 항등식으로 세 요인으로 분해합니다:

```
ln(국내가 변화) = ln(국제 USD가 변화) + ln(USD/KRW 변화) + ln(1+김프 변화)
R_dom(%)       = R_usd(%p)           + R_fx(%p)          + R_gap(%p, 잔차)
```

김프 요인(R_gap)은 잔차로 계산해 세 요인의 합이 국내 가격 변화와 **정확히 일치**합니다
(저장값 반올림 오차 흡수). USDT는 국제가가 ≈1 USD라 R_usd ≈ 0 — 사실상 환율 추종
자산임이 분해에서 그대로 드러납니다.
...
구조·품질 평가와 단계별 개선 계획은 [docs/project-review.html](docs/project-review.html) 참고. 리뷰 문서의 로드맵(P0~P2 리팩토링, 기능 제언)은 전부 반영 완료. 이후 후보: 알림 웹훅 시크릿 연결, 신규 자산(은·XRP 등), 분봉 데이터(아키텍처 전환 필요 — 보류 권장).
